...
public class AnimaToUnity : MonoBehaviour
{
    ...
    public static void SplitSprite()
    {
        ...
        foreach (var selectGO in selection)
        {
            if (selectGO is Texture2D)
            {
                ...
                string plistFile = texturePath.Replace(".png", ".plist");
    ...
    public static void CreateAnimClip()
    {
        ...
        foreach (var selectGO in selection)
        {
            ...
            var plistPathes = Directory.GetFiles(foldPath, "*.plist", SearchOption.TopDirectoryOnly);
            foreach (var plistFile in plistPathes)
            {
                animations.Add(BuildAnimationClip(plistFile, foldPath));
            }
        ...
        foreach (XmlNode keyNode in keys)
        {
            ...
            while (subKeyNode != null && subKeyNode.NextSibling != null)
            {
                ...
                if (subKeyNode.InnerText == "offset")
                {
                    ...
                    offsetX = (int)float.Parse(framStrs[0].Trim('{', '}'));
                    offsetY = (int)float.Parse(framStrs[1].Trim('{', '}'));
                ...
                if (subKeyNode.InnerText == "frame")
                {
                    ...
                    //frameStr = frameStr.Substring(2, frameStr.Length - 4);
                    string[] framStrs = frameStr.Split(',');
                    string strX = framStrs[0].Trim(' ', '{', '}');
                    string strY = framStrs[1].Trim(' ', '{', '}');
                    string strW = framStrs[2].Trim(' ','{','}');
                    string strH = framStrs[3].Trim(' ', '{', '}');
                    x = (int)(float.Parse(strX));
                    y = (int)(float.Parse(strY));
                    width = (int)(float.Parse(strW));
                    height = (int)(float.Parse(strH));
                }

                //if (subKeyNode.InnerText == "sourceColorRect")
                //{
                //    string frameStr = valueNode.InnerText;
                //    //frameStr = frameStr.Substring(2, frameStr.Length - 4);
                //    string[] framStrs = frameStr.Split(',');
                ...
                //    string strW = framStrs[2].Trim(' ', '{', '}');
                //    string strH = framStrs[3].Trim(' ', '{', '}');
                //    int sourcex = (int)(float.Parse(strX));
                //    int sourcey = (int)(float.Parse(strY));
                //    int sourcewidth = (int)(float.Parse(strW));
                //    int sourceheight = (int)(float.Parse(strH));

                //    offsetX =(int)(sourceheight * 0.5 + sourcex - height * 0.5);
                //    offsetY = (int)(sourcewidth * 0.5 + sourcey - width * 0.5);
                //}
                ...
                if (subKeyNode.InnerText == "rotated")
                {
                    if (valueNode.Name == "true")
                    {
                        ...
                        splitInfo.name = splitInfo.name + "(r)";
                    }
                }
            ...
            if (rotate)
            {
                ...
                splitInfo.pivot = new Vector2(((float)(width * 0.5f - offsetY)) / width, (height * 0.5f + offsetX) / height);
                //splitInfo.pivot = new Vector2((height * 0.5f - offsetY) / height, -(width * 0.5f - offsetX) / width);
                //splitInfo.pivot = new Vector2((width * 0.5f - offsetY) / width, (height * 0.5f + offsetX) / height);
            }
            else
            {
                //splitInfo.pivot = new Vector2((width * 0.5f - offsetX) / width, ( - offsetY) / height);
    ...
    static void BuildPrefab(string filePath, List<AnimationClip> anims)
    {
        ...
        PrefabUtility.CreatePrefab(filePath + "/" + go.name + ".prefab", go);
